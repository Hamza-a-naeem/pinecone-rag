...
chat_history = []  # Store last N pairs of messages

def add_to_history(role: str, content: str):
    """Store last 5 pairs of (user, assistant) messages."""
    global chat_history
...
def generate_response(question: str, context: str, llm_client, provider: str):
    """Generate a RAG response using Ollama and chat history."""
    ...
    system_prompt = (
        "You are a helpful assistant that answers user questions using provided "
        "company documents. If context lacks the answer, say so. "
        "You maintain conversational continuity with previous messages."
    )

    # Build manual prompt for Ollama
    history_text = ""
